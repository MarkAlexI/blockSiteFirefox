...
export const DIAGNOSTIC_BUILD = 'RC10';
...
export function formatDiagnosticReportText(report) {
  ...
  const lines = [
    ...
    `Page visibility: ${report.dailyLimits?.tracker?.visibilityState || 'not recorded'}`,
    `Visibility source: ${report.dailyLimits?.tracker?.visibilitySource || 'not recorded'}`,
    `Document focus: ${report.dailyLimits?.tracker?.documentHasFocus === true ? 'yes' : report.dailyLimits?.tracker?.documentHasFocus === false ? 'no' : 'unknown'}`,
