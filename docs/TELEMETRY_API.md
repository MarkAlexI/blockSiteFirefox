...
The schema-v2 counter allowlist currently includes rule, category, Rule List, Daily Limit, Free-limit, Focus Session, diagnostics, and feedback interaction counters. Product setup counters are limited to `rule_list_created`, `rule_list_activated`, `daily_limit_configured`, and `free_rule_limit_reached`. They contain no Rule List names, URLs, Daily Limit minutes, schedules, or arbitrary configuration values. Feedback counters are limited to `feedback_prompt_shown`, `feedback_review_clicked`, `feedback_support_clicked`, and `feedback_dismissed`; they contain no prompt content or free-form values.
