...
The schema-v2 counter allowlist currently includes rule, category, Rule List, Daily Limit, Free-limit, Focus Session, diagnostics, feedback interaction, and Firefox Pro activation counters. Product setup counters are limited to `rule_list_created`, `rule_list_activated`, `daily_limit_configured`, and `free_rule_limit_reached`. They contain no Rule List names, URLs, Daily Limit minutes, schedules, or arbitrary configuration values. Feedback counters are limited to `feedback_prompt_shown`, `feedback_review_clicked`, `feedback_support_clicked`, and `feedback_dismissed`; they contain no prompt content or free-form values. Pro activation counters are limited to `pro_activation_notice_shown`, `pro_activation_consent_requested`, `pro_activation_consent_granted`, and `pro_activation_succeeded`; they include no license key, account details, browsing history, or rule content, and are stored/sent only when optional technical analytics is enabled. `consent_requested` tracks the request attempt, not the rendering of a browser-native prompt.

Expected rule rejections such as invalid user input, schedule validation failures, rule conflicts, duplicate rules, Pro/free-limit checks, and invalid imports are not reported as reliability errors. The exact `rule_limit_reached` rejection may increment the aggregate `free_rule_limit_reached` product counter when telemetry is enabled. Unexpected rule mutation failures, DNR failures, storage/runtime failures, and uncaught errors remain eligible for privacy-safe error reporting.
